...
try {
    $lifeplanNo = trim($_GET["lifeplan_no"] ?? "");
    if (empty($lifeplanNo)) {
    ...
    $data["name"] = decryptIfEncrypted($data["applicant_name"] ?? '');
    $data["phone_no"] = decryptIfEncrypted($data["applicant_contact_no"] ?? '');
    $data["email"] = decryptIfEncrypted($data["applicant_email"] ?? '');
    $data["planholder_firstname"] = decryptIfEncrypted($data["planholder_firstname"] ?? '');
    $data["planholder_middlename"] = decryptIfEncrypted($data["planholder_middlename"] ?? '');
    $data["planholder_lastname"] = decryptIfEncrypted($data["planholder_lastname"] ?? '');
    $data["residential_address"] = decryptIfEncrypted($data["residential_address"] ?? '');
